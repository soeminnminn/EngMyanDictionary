...
interface DataAccess {
    ...
    suspend fun queryId(searchWord: String): Long

    @Query("SELECT _id FROM dictionary WHERE stripword IS :searchWord ORDER BY stripword ASC")
    suspend fun queryExacted(searchWord: String) : List<Long>
    ...
    suspend fun insertConverted(value: ConvertedItem): Long

    // Favorite
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertFavorite(value: FavoriteItem): Long

    @Query("DELETE FROM favorites WHERE _id IS :id")
    suspend fun deleteFavorite(id: Long)

    @Query("DELETE FROM favorites WHERE refrence_id IS :refId")
    suspend fun deleteFavoriteByRef(refId: Long)
    ...
    // History
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertHistory(value: HistoryItem): Long
