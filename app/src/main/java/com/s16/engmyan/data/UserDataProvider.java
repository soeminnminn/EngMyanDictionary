...
public class UserDataProvider extends AbstractDataProvider implements DataContents {
    ...
    static {
        for(int i=0; i<TABLES.length; i++) {
            ...
            URI_MATCHER.addURI(AUTHORITY, table.getTableName(), MAKELPARAM(table.getId(), DataContents.MATCH_ALL));
            URI_MATCHER.addURI(AUTHORITY, table.getTableName() + "/#", MAKELPARAM(table.getId(), DataContents.MATCH_ID));
        }
    }
    ...
    public String getType(Uri uri) {
        ...
        if (matchId != -1) {
            ...
            if (typeId == DataContents.MATCH_ID) {
